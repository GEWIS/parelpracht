...
async function authWithApiKey(apiKey: string) {
  ...
  const identity = (await AppDataSource.getRepository(IdentityApiKey).findOneBy({ apiKey: key }))!;

  if (identity === undefined) {
    throw new ApiError(HTTPStatus.Unauthorized, 'Unknown API Key');
  }

  return (await AppDataSource.getRepository(User).findOne({ where: { id: identity.id }, relations: ['roles'] }))!;
}
